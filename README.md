...
- flag Python issues such as blocking sync I/O in async code, swallowed broad exceptions, `eval` or `exec`, debug `print()` leftovers, `None` equality checks, side-effect comprehensions, redundant `return None`, hardcoded path strings, variadic public APIs, list materialization for a first element, queue-style list operations, textbook docstrings on tiny helpers, mixed naming conventions, repeated long string literals, large god-function shapes, monolithic `__init__.py` modules, oversized instance-attribute sets, full-file reads, and looped string concatenation
