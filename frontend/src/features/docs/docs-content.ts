...
const overviewContent = {
  go: {
    title: 'Go Analysis',
    lead: 'deslop ships its broadest heuristic surface area for Go. It walks the repository with .gitignore awareness, parses source structure with tree-sitter-go, builds a local package index keyed by package plus directory, and now combines repo-wide style checks, wrapper and goroutine coordination analysis, parser-backed Gin and GORM request-path detection, low-level string/slice/runtime/I/O performance idioms, crypto-auth-network security misuse, and library-specific Redis, gRPC, logging, config, Prometheus, and AWS client signals, alongside the opt-in deeper semantic loop pass with explainable rules.',
    bullets: [
      ...
      'Adds low-level performance findings for string formatting, slice and map growth, runtime sync primitives, decoder setup, error handling, and interface misuse',
      'Adds Go security findings for crypto misuse, JWT and cookie handling, injection sinks, TLS and network misconfiguration, data exposure, and concurrency-race hazards',
      'Covers library misuse across Redis, gRPC, structured logging, Viper and env config access, Prometheus metrics, Cobra flags, and AWS client calls',
